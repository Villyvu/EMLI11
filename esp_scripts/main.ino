...
volatile int red_status;
volatile int yellow_status;
volatile unsigned long count_prev_time;

// Wifi
#include <ESP8266WiFi.h>
const char* WIFI_SSID = "EMLI_TEAM_11";
      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          ...
          if (currentLine.length() == 0) {
            ...
            } else if (currentLine.startsWith("GET /led/a/red/status")) {
              Serial.println("Return red led status");
              sprintf ("%d", red_status);

            } else if (currentLine.startsWith("GET /led/a/yellow/status")) {
              Serial.println("Return yellow led status");
              sprintf ("%d", yellow_status);
